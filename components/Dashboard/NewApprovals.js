    ...
    return (
        <div className='pt-[30px]'>
            ...
            <div className='flex flex-row items-center space-x-[35px]'>
                <h1 className='text-xl font-medium text-misDarkBlack'>New Villa Approvals</h1>
            ...
            >
                <div className='flex flex-row items-center justify-between px-[25px]'>
                    ...
                    <Link href="/villas/pending_villas">
                        <h1 className='font-roboto text-[#08488C] text-sm cursor-pointer'>Pending Villa Owners {">"}</h1>
                    </Link>
                </div>
                    ...
                    <table className='table-fixed w-full'>
                        <thead >
                            <tr className='bg-[#08488C]'>
                                ...
                                <th className='py-[11px] text-xs text-white font-light text-start'>OWNER NAME</th>
                                <th className='py-[11px] text-xs text-white font-light text-start'>REQUEST DATE</th>
                        ...
                        <tbody>
                            {Array.isArray(data?.all?.data) && data?.all?.data.map((item, index) => {
                                return (
                                    ...
                                    >
                                        ...
                                        <td className='  py-[24px] text-[#54617A] font-roboto text-sm'>{item?.vendor_id}</td>
                                        <td className='  py-[24px] text-[#54617A] font-roboto text-sm '>{item?.city}</td>
                                        <td className='  py-[24px] text-[#54617A] font-roboto text-sm'>{item?.owner_name}</td>
                                        <td className='  py-[24px] text-black font-roboto text-sm'>{item?.request_date}</td>
                                        <td className={`py-[24px] font-medium font-roboto text-sm ${item?.status == 'yes' ? "text-green-500" : "text-red-500"}`}>{item?.status == 'yes' ? "Approved" : "Pending"}</td>
