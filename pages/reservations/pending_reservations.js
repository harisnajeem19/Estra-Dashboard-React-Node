...
const index = () =>
{
    ...
    const loadVillasList = async () =>
    {
        ...
        try {
            ...
            const { data } = await API.get('Dashboard/reservations/pending', header)
            setCardsData([
                {
                    title: 'Pending',
                    value: data?.body?.pending_reservations,
                    color: '#F52D56'
                }
            ])
            setPendingReservationsList(data?.body?.reservation_list?.data)
            setloading(false)
        } catch (error) {
            console.log(error)
            setloading(false)
    ...
    return (
        <>
            <Head>
                <title>Esterahy - Pending Reservations</title>
                <meta name="description" content="Generated by create next app" />
                <link rel="icon" href="/favicon.ico" />
            </Head>

            <Layout title="Pending Reservations" slug="Explore all villa reservations here">
                {/* Cards  Start*/ }
                ...
                <div className='mt-[20px] pb-12'>
                    <table className='table-fixed w-full rounded-lg' style={ { 'boxShadow': '0px 0px 20px #D7DEE365' } }>
                        ...
                        <tbody>
                            { pendingReservationsList?.map((item, index) =>
                            {
                                    ...
                                    >
                                        <tr className='border-b last:border-b-0 border-[#EFF3F9] cursor-pointer'>
                                            ...
                                            <td className='  py-[24px] text-[#54617A] font-roboto text-sm cursor-pointer'>
                                                <Link href={ `/villas/villa_details/${[item?.id]}` }>
